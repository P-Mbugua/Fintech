    ...
    <div className="flex justify-center items-center p-4 bg-blue-600">
      <div className="w-full max-w-6xl bg-white rounded-lg shadow-md">
        ...
        <div className="sm:hidden p-4">
          <h2 className="text-lg font-bold mb-2 text-center">Featured Categories</h2>
          <div className="flex overflow-x-auto space-x-4 scrollbar-hide p-2 bg-gray-100 rounded-lg shadow-md">
            {departments.map((dept, index) => (
              <div key={index} className={`flex-none w-24 text-center transition-all duration-500 ${scrollIndex === index ? "opacity-100" : "opacity-50"}`}>
                <button className="flex flex-col items-center p-2 bg-white rounded-lg shadow hover:bg-gray-300 transition duration-300">
                  <span className="text-3xl">{dept.icon}</span>
                  <span className="text-sm mt-1 font-semibold">{dept.name}</span>
                </button>
              </div>
            ))}
          </div>
        ...
        {/* Navigation Tabs */}
        <div className="overflow-x-auto flex bg-gray-200 whitespace-nowrap p-2 rounded-b-lg">
          {[
            "ALOT For Less", "New Arrivals", "Summer", "Fire Sale", "Small Local Sellers", "Brands Store", "Clearance"
          ].map((item, index) => (
            <button
              key={index}

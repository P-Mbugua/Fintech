      ...
      <div className="w-full max-w-7xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((product) => (
          ...
          >
            ...
            <p
              className={`text-sm ${product.stock > 5 ? "text-gray-600" : "text-red-600 font-bold"}`}
            >
            ...
            <div className="flex justify-center gap-3 mt-3">
              <button
                ...
                className="bg-blue-600 text-white text-sm px-4 py-2 rounded-md hover:bg-blue-800 shadow-md flex items-center gap-2"
              >
                <FaShoppingCart /> Add to Cart
              </button>
              <button
                onClick={() => handleOrderNow(product)}
                className="bg-green-600 text-white text-sm px-4 py-2 rounded-md hover:bg-green-800 shadow-md flex items-center gap-2"
              >
                <FaMoneyBillWave /> Order Now
              </button>
            </div>
          </div>
        ))}

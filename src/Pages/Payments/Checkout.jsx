...
const handlePlaceOrder = async () => {
  ...
  const totalAmount = cart.reduce(
    (acc, item) => acc + (item.price01 || item.price) * (item.quantity || 1),
    0
  ) + 199;
  ...
  try {
    ...
    for (const item of cart) {
      try {
        ...
        console.error(`Failed to delete cart item ${item.$id}:`, delErr);
      }
    }

    // ✅ Clear cart locally AND refresh context
    clearCart();        // clears cart in CartContext
    setCart([]);        // clears local state for immediate refresh
  ...
  if (loadingUser) return <div className="text-center mt-10 text-gray-500">Loading...</div>;
  ...
  return (
    <div className="checkout-container bg-white p-6 sm:p-8 md:p-10 rounded-2xl shadow-2xl max-w-5xl mx-auto relative font-sans text-gray-800 mt-8 md:mt-14 lg:mt-14">
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-10 text-center text-green-500">
        Checkout
      </h1>
      ...
      <section className="mb-10">
        ...
        <select
          className="border-2 border-gray-400 p-3 rounded-xl w-full text-gray-800 text-sm sm:text-base focus:ring-2 focus:ring-green-500 transition-all duration-300 hover:border-green-500"
      ...
      <section className="mb-10">
        <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-4">Product List Summary</h2>
            ...
            cart.map((item, index) => (
              ...
              >
                <div className="flex items-center space-x-4 mb-3 sm:mb-0">
                  <img src={item.image} alt={item.productName || item.name} className="w-16 h-16 sm:w-24 sm:h-24 object-cover rounded-lg" />
      ...
      <section className="mb-10 p-5 bg-gray-100 rounded-xl shadow-inner text-sm sm:text-base md:text-lg">
        <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-4">Total</h2>
        ...
        <div className="flex justify-between border-t border-gray-400 pt-2 text-lg sm:text-xl font-bold text-green-500">
